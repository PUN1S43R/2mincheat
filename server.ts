...
import { Document, Packer, Paragraph, AlignmentType, TextRun, Header } from "docx";
...
async function processTextToFiles(text: string, originalName: string, originalFile: string, isDarkMode: boolean = false) {
  ...
  const generateDocx = async () => {
    const watermarkText = "TECHSZDEVELOPER";
    const doc = new Document({
      background: isDarkMode ? { color: "141414" } : undefined,
      sections: [{
        properties: {
          page: {
            size: {
          ...
          default: new Header({
            children: [
              new Paragraph({
                children: [
                  new TextRun({
                    text: watermarkText + " ".repeat(10) + watermarkText,
                    bold: true,
                    size: 24,
  ...
  const generatePdf = async () => {
    ...
    const textColor = isDarkMode ? rgb(0.894, 0.89, 0.878) : rgb(0.078, 0.078, 0.078); // #E4E3E0 vs #141414
    const bgColor = isDarkMode ? rgb(0.078, 0.078, 0.078) : rgb(0.894, 0.89, 0.878);
    const watermarkColor = isDarkMode ? rgb(0.4, 0.4, 0.4) : rgb(0.6, 0.6, 0.6);
    ...
    const watermarkText = "TECHSZDEVELOPER";
    const drawWatermarks = (p: any) => {
      const color = isDarkMode ? rgb(1, 1, 1) : rgb(0, 0, 0);
      const opacity = 0.08;
      const size = 35;
      ...
      for (let i = 0; i < allWords.length; i++) {
        ...
        if (currentLineWidth + word.width + spaceWidth > contentWidth && currentLine.length > 0) {
          ...
          for (let j = 0; j < currentLine.length; j++) {
            ...
            page.drawText(item.sanitizedText, { x: xOffset, y, size: fontSize, font: item.font, color: textColor });
...
app.post("/api/convert", upload.fields([{ name: 'reference' }, { name: 'document' }]), async (req, res) => {
  try {
    ...
    const docId = await processTextToFiles(text, docFile.originalname, docFile.filename, isDarkMode);
...
app.post("/api/convert-text", async (req, res) => {
  try {
    const { text, isDarkMode } = req.body;
    if (!text) {
      return res.status(400).json({ error: "No text provided" });
    }

    const docId = await processTextToFiles(text, "Pasted Text Content", "N/A", !!isDarkMode);
    res.json({ success: true, id: docId });
  } catch (error: any) {
